...
const tTypeValides = ["ARME","ARME_2_MAINS","MUNITION","BOUCLIER","ARMURE","BOTTES","CASQUE","GANT","CAPE","BIJOU","TRINKET","POTION","PARCHEMIN","BAGUETTE","BATON","FOCALISATEUR","OUTIL","SAC","CONTENEUR","MARCHANDISE","AUTRE"];
const typeDegatsValides = ["FEU","GLACE","ACIDE","POISON","TONNERRE","PSYCHIQUE","NECROTIQUE","FORCE","PERCANT","CONTONDANT","TRANCHANT","RADIANT"];
...
routeurObjets.post("/objet/creer", authentifier, exigerRole("MAITRE_DU_JEU"), async (req: Request, res: Response) => {
    const {nom, rarete, tType, prix} = req.body;

    if (!nom) return res.status(400).json({ erreur: "Erreur: Le nom de l'objet est manquant" });
    if (!raretesValides.includes(rarete)) return res.status(400).json({erreur: `Erreur: La rareté de l'objet est invalide`})
    if (!tTypeValides.includes(tType)) return res.status(400).json({erreur: `Erreur: Le type de l'objet est invalide`})
    if (typeof prix != "number") return res.status(400).json({erreur: `Erreur: Le prix doit être un nombre`})


    const data : Objet = { nom, rarete, tType, prix}

    if (req.body.att !== undefined && ["ARME", "ARME_2_MAINS"].includes(req.body.tType)){
            if (typeof req.body.att !== "number") return res.status(400).json({ erreur: "L'attaque est invalide"})
            data.att = req.body.att
        }
    if (req.body.def !== undefined && ["ARMURE", "BOTTES", "CASQUE", "BOUCLIER", "GANT"].includes(req.body.tType)){
            if (typeof req.body.def !== "number") return res.status(400).json({ erreur: "La defense est invalide"})
...
routeurObjets.patch("/objet/:nom", authentifier, exigerRole("MAITRE_DU_JEU"), async(req: Request, res: Response)=>{
    ...
    try{
        ...
        if (body.tType !== undefined ){
            if (!tTypeValides.includes(body.tType)){
                return res.status(400).json({ erreur: "Type d'objet invalide" });
            } data.tType = body.tType;
        } 
        ...
        const typeObjet = body.tType || objet.tType
        if (body.att !== undefined && ["ARME", "ARME_2_MAINS"].includes(typeObjet)){
            if (typeof body.att !== "number") return res.status(400).json({ erreur: "L'attaque est invalide"})
            data.att = body.att
        }

        if (body.def !== undefined && ["ARMURE", "BOTTES", "CASQUE", "BOUCLIER", "GANT"].includes(typeObjet)){
            if (typeof body.def !== "number") return res.status(400).json({ erreur: "La defense est invalide"})
            data.def = body.def
        }
        if (body.prix !== undefined){
            if (typeof body.prix !== "number") return res.status(400).json({ erreur: "Le prix doit être un nombre"})
            data.prix = body.prix
        }
        ...
        if (body.degatsBonus !== undefined ){
            if (typeof body.degatsBonus !== "number") return res.status(400).json({ erreur: "Type de dégats bonus invalide"})
